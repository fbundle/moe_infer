"""Same head-to-head as bench_anemll_vs_mlx.py but for the 3B class.

Anemll's Hermes-3.2-3B (Llama-3.2 family) on ANE vs MLX 4bit Hermes-3 3B
on GPU. Same prompt, same decode length, same /tmp/power.log.
"""
...
def run_anemll_3b(prompt: str, max_tokens: int) -> RunResult:
    repo = Path(__file__).resolve().parents[1]
    venv = repo / ".venv-anemll" / "bin" / "python"
    snap = next((Path(os.path.expanduser(
        "~/coreml_models/models--anemll--anemll-Hermes-3.2-3B-iOS-0.1.1/snapshots"
    ))).iterdir())
    ...
    print(f"[Anemll/ANE] Hermes 3B  running ...")
    start = time.time()
    out = subprocess.run(cmd, capture_output=True, text=True, timeout=900)
    end = time.time()
    text = out.stdout + out.stderr
    dec = float(RE_ANEMLL_INFER.search(text).group(1)) if RE_ANEMLL_INFER.search(text) else None
    pre = float(RE_ANEMLL_PREFILL.search(text).group(1)) if RE_ANEMLL_PREFILL.search(text) else None
    print(f"             decode={dec}  prefill={pre}  wall={end-start:.2f}s")
    return RunResult("Anemll/ANE (Hermes-3 3B)", start, end, dec, pre)


def run_mlx_3b(prompt: str, max_tokens: int) -> RunResult:
    repo = Path(__file__).resolve().parents[1]
    venv = repo / ".venv" / "bin" / "python"
    mdir = next((Path(os.path.expanduser(
        "~/coreml_models/models--mlx-community--Hermes-3-Llama-3.2-3B-4bit/snapshots"
    ))).iterdir())
    code = f"""
import time, json
...
"""
    print(f"[MLX-LM/GPU] Hermes 3B  running ...")
    start = time.time()
    out = subprocess.run([str(venv), "-c", code], capture_output=True, text=True, timeout=900)
    end = time.time()
    dec = pre = None
    for line in out.stdout.splitlines():
        ...
        if line.startswith("{") and line.endswith("}"):
            try:
                ...
                pass
    print(f"             decode={dec}  prefill={pre}  wall={end-start:.2f}s")
    return RunResult("MLX-LM/GPU (Hermes-3 3B)", start, end, dec, pre)
...
def main():
    ...
    anemll = run_anemll_3b(args.prompt, args.max_tokens)
    time.sleep(args.gap_s)
    mlx = run_mlx_3b(args.prompt, args.max_tokens)
    time.sleep(args.gap_s)
