/// Qwen3.6-35B-A3B-4bit Fused4bitExp2 engine — all model dimensions are compile-time constants.
...
pub struct Fused4bitExp2<C: ModelConfig> {
    pub model: Arc<Model>,
...
impl<C: ModelConfig> Fused4bitExp2<C> {
    pub fn new(model: Arc<Model>, k: usize) -> Result<Self, MoEError> {
        C::validate_config(&model.config).map_err(MoEError::Config)?;
        let (ctx, weight_buffer, expert_buffer) = MetalContext::new::<C>(&model.weight_file, k, "Fused4bitExp2")?;
        Ok(Fused4bitExp2 {
...
impl<C: ModelConfig> Engine for Fused4bitExp2<C> {
