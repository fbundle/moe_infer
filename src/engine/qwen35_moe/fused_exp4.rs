//! Qwen3.6 FusedExp4 — FusedExp3 + persistent batched buffers.
//!
//! New optimization vs FusedExp3: the `BatchedFullBuffers` (N-sized scratch
//! for the layer-batched compute) and `ExpertPool` (per-unique-expert
//! Metal buffers, ~450 MB at full Qwen3.6 size) are allocated ONCE per
//! engine lifetime and reused across every `forward_hidden_batched` call.
//!
//! FusedExp3 allocates these fresh on every call — a ~450 MB alloc + GPU
//! buffer creation, costing ~50-100 ms per call. For chat / prefill
//! workloads with repeated calls, that overhead compounds.
//!
//! Reuse policy:
//!   - On first call: allocate buffers sized for the current N.
//!   - On subsequent calls:
//!       if N ≤ N_cached: reuse (the kernels write only the first
//!         N slots of each buffer; the remainder stays stale but unused).
//!       if N > N_cached: grow — reallocate at the new N (also rare).
//!   - The ExpertPool is N-independent (sized by num_experts), so it's
//!     allocated once and never grown.
//!
//! Selectable via pipeline_mode="Qwen35MoEFusedExp4".
//!
//! Bit-near-identical to FusedExp3 on logits (max_diff ~2e-5, top-1 match)
//! — the batched compute is unchanged, only the buffer lifecycle changes.
...
use crate::constants::{FULL_ATTN_INTERVAL, MAX_SEQ};
use crate::engine::{Engine, EngineSnapshot, SignalCheckFn, TelemetryValue};
use crate::engine::metal_context::MAX_K;
...
use crate::math::{normalize_weights, softmax, topk};
use crate::model::Model;

// Local copies of the same helpers fused_exp3 uses. Kept private; reused
// here so fused_exp4 doesn't depend on fused_exp3 (different overrides).
use crate::engine::fused_exp3::FusedExp3;
// We will use the public-ish helpers from fused_exp3 indirectly by calling
// the same batched.rs primitives. Local helper functions below mirror
// fused_exp3's private ones (route_cpu_only, pread_unique_experts,
// gpu_lm_head_n, final_norm) to keep this file self-contained.
...
fn final_norm(wf: &WeightFile, hidden: &mut [f32], hidden_dim: usize) {
    ...
    let inv_rms = 1.0 / (sum_sq / hidden_dim as f32 + crate::constants::RMS_NORM_EPS).sqrt();
...
pub struct FusedExp4<C: ModelConfig> {
    pub inner: FusedExp2<C>,
    /// Cached batched buffers sized for the largest N seen so far.
    /// `None` until the first forward_hidden_batched call.
    cached_bufs: Option<(usize, BatchedFullBuffers)>,
    /// Cached unique-expert pool. Sized by num_experts (N-independent),
    /// allocated on first batched call, never grown.
    cached_pool: Option<ExpertPool>,
    _phantom: PhantomData<C>,
}

impl<C: ModelConfig> FusedExp4<C> {
    ...
    /// Get or allocate the batched buffers. Grows on N > N_cached; reuses on N ≤ N_cached.
    /// Returns a reference to the cached buffers (lifetime tied to &self).
    fn get_or_alloc_bufs(&mut self, n: usize) -> &BatchedFullBuffers {
        let needs_realloc = match &self.cached_bufs {
            Some((cached_n, _)) => n > *cached_n,
            None => true,
        };
        if needs_realloc {
            let bufs = BatchedFullBuffers::new::<C>(&self.inner.ctx.device, n);
            self.cached_bufs = Some((n, bufs));
        }
        &self.cached_bufs.as_ref().unwrap().1
    }

    /// Get or allocate the unique-expert pool. Allocated once at first call.
    fn get_or_alloc_pool(&mut self) -> &ExpertPool {
        if self.cached_pool.is_none() {
            let num_experts = C::NUM_EXPERTS;
            let expert_size = self.inner.model.expert_files[0].expert_size();
            self.cached_pool = Some(ExpertPool::new(&self.inner.ctx.device, num_experts, expert_size));
        }
        self.cached_pool.as_ref().unwrap()
    }
}

impl<C: ModelConfig> Engine for FusedExp4<C> {
    ...
    fn forward_hidden_batched(
        ...
        // with calling other &self methods later are avoided.
        let _ = self.get_or_alloc_bufs(n);
        let _ = self.get_or_alloc_pool();
        // Now grab references — these live as long as &self below.
        let bufs = &self.cached_bufs.as_ref().unwrap().1;
        let pool = self.cached_pool.as_ref().unwrap();

        // Seed bufs.hidden_n with the input embeddings. (Buffer may be
        // sized larger than n if cached from a previous larger call —
        // we only write the first n slots, rest is stale but unused.)
        unsafe {
            std::ptr::copy_nonoverlapping(
                embeddings.as_ptr(),
    ...
    fn snapshot(&self) -> EngineSnapshot {
        // Delegate to FusedExp3's snapshot semantics (same struct contents).
        // Easier than reimplementing: temporarily wrap inner in a FusedExp3
        // to call its snapshot, then return. But FusedExp3::snapshot is
        // private to that file, so reimplement here.
...
// Suppress the unused-import warning for FusedExp3 (kept around because the
// docs above reference it conceptually).
#[allow(unused_imports)]
use crate::engine::fused_exp3 as _docs_fused_exp3;
