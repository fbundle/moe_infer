/// Qwen3.6-35B-A3B-BQ4 FusedExp5 engine — iteration 5.
///
/// Inherits FusedExp1's pipeline structure. The optimization vs FusedExp1:
/// each top-k expert's gate_proj + up_proj + SwiGLU is fused into a single
/// kernel dispatch (`fused_gate_up_swiglu_v3`) instead of three. The shared
/// memory cache for x is loaded once per expert; gate and up matvecs run in
/// lockstep over the cached input, then SwiGLU is applied before writing
/// `expert_scratch_act` for the down_proj.
///
/// Effect on M4: ~+12-15% decode throughput on Qwen3.6-35B-A3B vs FusedExp1.
...
fn encode_post_expert<C: ModelConfig>(
    ...
    for ki in 0..actual_k {
        ...
        // expert_scratch_gate / expert_scratch_up are unused in FusedExp5.
...
pub struct FusedExp5<C: ModelConfig> {
    pub model: Arc<Model>,
...
impl<C: ModelConfig> FusedExp5<C> {
    pub fn new(model: Arc<Model>, num_active_experts: usize, expert_cache_count: usize) -> Result<Self, MoEError> {
        ...
        let (ctx, weight_buffer, expert_buffer) = MetalContext::new::<C>(&model.weight_file, num_active_experts, "FusedExp5", expert_cache_count)?;
...
impl<C: ModelConfig> Engine for FusedExp5<C> {
