/// Qwen3.6-35B-A3B-BQ4 FusedExp6 engine — iteration 6.
///
/// Builds on FusedExp5 (fused gate+up+SwiGLU). The new optimization here:
/// **async expert prefetch**. After op2 of layer L is encoded, the main
/// thread immediately schedules background rayon tasks that pread experts
/// L+1 will probably need, predicted as "same expert indices as L." Those
/// reads overlap with the GPU's command buffer execution of op2[L] +
/// op1[L+1]. By the time route_experts(L+1) runs, hits in the prefetch pool
/// are zero-latency.
///
/// Prefetch pool: 8 slots × ~1.6 MB = ~13 MB. Intentionally tiny so it
/// doesn't fight the OS page cache for the mmap'd expert directory the way
/// the existing LRU does at large sizes.
///
/// Effect on M4: TBD on bench.
...
use crate::engine::expert_prefetch::PrefetchPool;
...
struct Routing {
    ...
    /// Expert indices for this layer. Kept so op2 can speculatively prefetch
    /// the same indices for layer+1 (high cross-layer routing correlation).
...
impl<'b, C: ModelConfig> ExecCtx<'b, C> {
    ...
    fn route_experts(&mut self, layer: usize, mut gate_scores: GateScores) -> Routing {
...

        {
            ...
            // Phase 1a: speculative prefetch pool — zero-latency hits for
            // experts we already pre-paged in during op2[L-1].
            ...
            for ki in 0..actual_k {
                ...
                let mut hit = None;
                for pool in &self.engine.prefetch_pools {
                    if let Some(buf) = pool.lookup(layer, eidx) {
                        hit = Some(buf);
                        break;
                    }
                }
            ...
            timing_add(&mut self.engine.timing, "prefetch.hits",  prefetch_hits as f64);
            timing_add(&mut self.engine.timing, "prefetch.checks", actual_k    as f64);

            // Phase 1b: fall through to the shared LRU cache for anything the
            // prefetch pool didn't already satisfy.
            {
                let expert_buf = &mut self.engine.expert_buffer;
                if let Some(ref mut cache) = expert_buf.cache {
                    let mut still_missing: Vec<(usize, usize)> = Vec::with_capacity(misses.len());
                    for &(ki, eidx) in &misses {
                        if let Some(buf) = cache.lookup(layer, eidx) {
                            resolved_data[ki] = Some(buf);
                        } else {
                            still_missing.push((ki, eidx));
                        }
                    }
        ...
        Routing {
            ...
            expert_data,
            expert_indices: expert_indices.iter().take(actual_k).copied().collect(),
        }
    }
    ...
    fn op2(&mut self, layer: usize, routing: &Routing) {
        ...
        // ── Speculative prefetch for layer+1 ──
        // Target pools[(L+1) % 2]. The previous fill of that pool was for
        // layer (L-1), consumed by op2(L-1), which was committed and waited
        // by op1_wait(L). So pool[(L+1)%2] is guaranteed not in flight.
        if layer + 1 < C::NUM_LAYERS {
            let next_layer = layer + 1;
            let pool_idx = next_layer % self.engine.prefetch_pools.len();
            self.engine.prefetch_pools[pool_idx].schedule(
...
fn encode_post_expert<C: ModelConfig>(
    ...
    for ki in 0..actual_k {
        ...
        // gate_proj + up_proj + SwiGLU in ONE dispatch.
        // expert_scratch_gate / expert_scratch_up are unused in FusedExp6.
        metal_kernels::encode_fused_gate_up_swiglu(ctx, enc,
            ...
            post_normed, 0,
            expert_scratch_act, 0,
            inter_u32, hidden_u32, gs_u32);
...
pub struct FusedExp6<C: ModelConfig> {
    pub model: Arc<Model>,
    pub ctx: MetalContext,
    pub weight_buffer: WeightBuffer,
    pub expert_buffer: ExpertBuffer,
    pub num_active_experts: usize,
    /// Two alternating prefetch pools. Schedule(L) writes to pools[L % 2].
    /// Consumer of pool[X] for layer L is op2(L); op2(L-2) (the previous
    /// consumer of the same pool) is guaranteed done because it was
    /// committed-and-waited by op1_wait(L-1) before we ever fill pool[X]
    /// again. No race with in-flight GPU work.
    pub prefetch_pools: Vec<PrefetchPool>,
    pub timing: BTreeMap<String, TelemetryValue>,
    _phantom: PhantomData<C>,
}

impl<C: ModelConfig> FusedExp6<C> {
    pub fn new(model: Arc<Model>, num_active_experts: usize, expert_cache_count: usize) -> Result<Self, MoEError> {
        ...
        let (ctx, weight_buffer, expert_buffer) = MetalContext::new::<C>(&model.weight_file, num_active_experts, "FusedExp6", expert_cache_count)?;
        let prefetch_pools = vec![
            PrefetchPool::new(&ctx.device, MAX_K, C::EXPERT_SIZE_4BIT),
            PrefetchPool::new(&ctx.device, MAX_K, C::EXPERT_SIZE_4BIT),
        ];
        ...
        Ok(FusedExp6 {
            model, ctx, weight_buffer, expert_buffer, prefetch_pools,
...
impl<C: ModelConfig> Engine for FusedExp6<C> {
