...
pub struct Fused4bit<C: ModelConfig> {
    pub model: Arc<Model>,
...
impl<C: ModelConfig> Fused4bit<C> {
    pub fn new(model: Arc<Model>, k: usize) -> Result<Self, MoEError> {
        C::validate_config(&model.config).map_err(MoEError::Config)?;
        let (ctx, weight_buffer, expert_buffer) = MetalContext::new::<C>(&model.weight_file, k, "Fused4bit")?;
        Ok(Fused4bit {
...
impl<C: ModelConfig> Engine for Fused4bit<C> {
