...
// Model configuration — generated by gen_model_config.py
#include "model_config.h"

#define MAX_ACTIVE_EXPERTS 64

// main.m compatibility: this file uses _OFFSET suffix instead of _OFF
...
int main(int argc, char **argv) {
    @autoreleasepool {
        ...
        int c;
        while ((c = getopt_long(argc, argv, "l:e:bmFk:vfp:j:h", long_options, NULL)) != -1) {
            switch (c) {
        ...
        // Config from compile-time model_config.h
        printf("[config] hidden=%d, inter=%d (MOE_INTERMEDIATE), experts=%d, layers=%d, expert_size=%zu\n",
               HIDDEN_DIM, MOE_INTERMEDIATE, NUM_EXPERTS, NUM_LAYERS, (size_t)EXPERT_SIZE);
        ...
        printf("=== metal_infer: 4-bit dequant MoE engine (config-driven) ===\n");
        if (do_full) {
            printf("Mode: FULL %d-layer forward, K=%d, Shader: %s, Benchmark: %s\n",
                   NUM_LAYERS, num_active_experts, shader_name,
                   do_benchmark ? "YES" : "NO");
        } else {
